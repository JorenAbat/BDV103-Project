...
app.use(async (ctx, next) => {
  if (ctx.path === '/swagger.json') {
    // Try to serve the combined swagger.json from build directory
    const combinedSwaggerPath = '/app/build/swagger.json';
    try {
      const fs = await import('fs');
      if (fs.existsSync(combinedSwaggerPath)) {
        const combinedSwagger = JSON.parse(fs.readFileSync(combinedSwaggerPath, 'utf8'));
        ctx.type = 'application/json';
        ctx.body = combinedSwagger;
        return;
      }
    } catch (error) {
      console.warn('Could not load combined swagger.json, falling back to basic documentation:', error instanceof Error ? error.message : String(error));
    }
    
    // Fallback to basic swagger documentation
    ctx.type = 'application/json';
