...
async function testEventSubscriptions() {
  ...
  try {
    ...
    const addBookResponse = await axios.post(`${BASE_URL}/books`, bookData);
    ...
    await axios.post(`${BASE_URL}/warehouse/add-books`, stockData);
    ...
    const bookResponse = await axios.get(`${BASE_URL}/books/${bookId}`);
    const book = bookResponse.data;
    console.log(`📊 Book stock in books service: ${book.totalStock}`);
    
    if (book.totalStock === 50) {
    ...
    const warehouseResponse = await axios.get(`${BASE_URL}/warehouse/${bookId}`);
    ...
    await axios.put(`${BASE_URL}/books/${bookId}`, updatedBookData);
    ...
    await axios.post(`${BASE_URL}/warehouse/remove-books`, removeStockData);
    ...
    const finalBookResponse = await axios.get(`${BASE_URL}/books/${bookId}`);
    const finalBook = finalBookResponse.data;
    console.log(`📊 Final book stock: ${finalBook.totalStock}`);
    
    if (finalBook.totalStock === 40) {
